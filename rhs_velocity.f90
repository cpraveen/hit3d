...
!-------------------------------------------------------------------------
  ...
  ! which are not yet multiplyed by "i"
...
!!$        if (wrk(i,j,k,4) .gt. t1(5) ) then 
        if (akx(i)**2 + aky(k)**2 + akz(j)**2 .le. kmax**2) then
          ! RHS for u, v and w
          do n = 1,3
          ...
          ! dealiasing: making everything higher than kmax zero
          wrk(i  ,j,k,1:3) = zip
          wrk(i+1,j,k,1:3) = zip
        end if
